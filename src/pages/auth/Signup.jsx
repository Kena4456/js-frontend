...
const Tagline = styled.p`text-align: center; color: var(--text-tertiary); font-size: 14px; margin-bottom: 32px;`;
...
`;
const RoleGroup = styled.div`display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 22px;`;
const RoleBtn = styled.button`
  padding: 11px; border-radius: 24px; font-size: 14px; font-weight: 600;
  border: none; transition: all 200ms ease;
  background: ${p => p.$active ? 'var(--accent)' : 'var(--bg-subtle)'};
  color: ${p => p.$active ? '#fff' : 'var(--text-secondary)'};
  &:hover { background: ${p => p.$active ? 'var(--accent-hover)' : 'var(--border)'}; }
`;

const Btn = styled.button`
  width: 100%; padding: 0; height: 44px; margin-top: 4px;
...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.email || !form.password || !form.username) {
      return setError('Email, password, and username are required.');
    }
    setLoading(true);
    try {
      ...
      if (err.code === 'auth/email-already-in-use' || err.message?.includes('Email already')) {
      ...
      <Card>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Field>
            ...
            <Input value={form.username} onChange={set('username')} placeholder="janedoe" required />
          </Field>
          <Field>
            <Label>Email</Label>
          ...
          <RoleGroup>
            <RoleBtn type="button" $active={form.role === 'student'} onClick={() => setForm(f => ({ ...f, role: 'student' }))}>Student</RoleBtn>
            <RoleBtn type="button" $active={form.role === 'teacher'} onClick={() => setForm(f => ({ ...f, role: 'teacher' }))}>Teacher</RoleBtn>
          </RoleGroup>
          {error && <ErrMsg>{error}</ErrMsg>}
          <Btn type="submit" disabled={loading}>{loading ? 'Creating account...' : 'Create Account'}</Btn>
        </form>
        <BottomLink>Already have an account? <Link to="/login">Sign in</Link></BottomLink>
      </Card>
    </Wrap>
  );
}
